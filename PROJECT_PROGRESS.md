...
> **文档版本**: v1.4
> **最后更新**: 2026-03-18
> **项目状态**: ✅ v1.3.0 核心功能修复完成，⚠️ 后端故障添加API待修复
...
### 问题11: 故障添加API返回500错误 ❌ 待后端修复
- **现象**: 提交故障信息时后端返回 `500 Internal Server Error`
- **原因**: 后端 `/faults/addinfo` 接口存在Bug
- **详情**: 见 `D:\桌面\智能路灯-后端故障添加API问题交接文档.md`
- **临时方案**: 无（需后端开发者修复）
- **发现的后端问题**:
  - 存在两套重复的故障控制器：`/fault/*` 和 `/faults/*`
  - 两个数据模型字段不一致：`Fault` 和 `Faults`
  - `Fault` 模型有字段拼写错误：`creatTime` 应为 `createTime`
...
| 故障管理 | ⚠️ 部分正常 | 详情查看、状态更新正常；**添加功能后端待修复** |
| 建议管理 | ✅ 正常 | 待处理、已处理均已修复 |
...
### 紧急优先级（后端Bug）
- [ ] **修复故障添加API** - `/faults/addinfo` 返回500错误（详见交接文档）
- [ ] 整合后端重复控制器（`/fault/*` 和 `/faults/*`）
- [ ] 统一数据模型字段命名（修复 `creatTime` 拼写错误）
