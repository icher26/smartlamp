...
### v1.7.0 (2026-04-04) — 用户注销功能修复

#### 修复内容
- [x] 彻底解决用户注销后白屏、卡死、无法重新登录的问题
- [x] 解决注销后重新登录停留在菜单界面的问题
- [x] 使用 `getWebviewById('index')` 替代不可靠的 `opener()` 获取主页面引用
- [x] 简化注销流程：直接打开登录页后关闭主页面
- [x] 简化登录流程：优化页面跳转时序
- [x] 移除不再需要的 `logoutEvent` 和 `reRoute` 事件处理代码

#### 涉及文件（4个）
| 文件 | 修改类型 | 说明 |
|------|----------|------|
| `offcanvas-drag-left-plus-menu.html` | 重构 | 简化注销逻辑，优化页面关闭时序 |
...
### v1.6.0 (2026-04-04) — 亮灯时段分布图表重构

#### 新增功能
- [x] 接入新API `/lamp-stats/lighting/time-distribution` 获取亮灯时段分布数据
- [x] 重构24小时点亮统计图表，改用柱状图展示
- [x] 待处理建议添加长按标记已处理功能
- [x] 移除建议详情页刷新按钮（功能转移至长按菜单）

#### 图表优化
- [x] X轴标签45度倾斜显示偶数时间段（00-01, 02-03...）
- [x] 柱子宽度优化，保留合理间隙
- [x] 柱子与时间轴精确对齐（boundaryGap: false）
- [x] 非零值柱子顶部显示数值标签
- [x] Tooltip显示完整时间段信息
...
| 文件 | 修改类型 | 说明 |
|------|----------|------|
| `js/config.js` | 新增端点 | 添加 lampStatsTimeDistribution 端点 |
| `lamp_details.html` | 图表重构 | 接入新API，优化图表样式 |
| `pending_advice.html` | 功能新增 | 添加长按标记已处理功能 |
| `advice_details.html` | 功能移除 | 移除刷新按钮 |
